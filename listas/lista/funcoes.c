...
// Adiciona um elemento depois do iterador
void anxLista(Lista lst, TipoL elem)
{
  ...
  else
  {
    ...
    else // Iterador sobre um No intermediario
    {
      // O prox do novoNo aponta para o proximo No depois do iterador
...
void insLista(Lista lst, TipoL elem)
{
  ...
  else
  {
    ...
    else // Iterador sobre qualquer outro No
    {
      ...
      // Ao sair do for, p esta um No antes do iterador
...
// Elimina o elemento que está sob o iterador
...
int iguaisListas(Lista lst1, Lista lst2)
{
  ...
  if (longLista(lst1) == 0 && longLista(lst2) == 0)
    return 1;
  // Senão se long de lst1 == long lst2
